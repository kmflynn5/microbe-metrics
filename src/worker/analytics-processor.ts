...
export class AnalyticsProcessor {
	...
	private async computeOverview(projects: JGIGenomeProject[]) {
		const archaeaProjects = projects.filter((p) => p.metadata.domain === "Archaea").length;
		const bacteriaProjects = projects.filter((p) => p.metadata.domain === "Bacteria").length;

		// Get historical data for growth rate calculation
		const historicalData = await this.getHistoricalOverview();
		const growthRate = this.calculateGrowthRate(projects.length, historicalData);

		// Calculate new projects this week
		const weekAgo = new Date();
		weekAgo.setDate(weekAgo.getDate() - 7);
		const newProjectsThisWeek = projects.filter((p) => new Date(p.submissionDate) > weekAgo).length;
	...
	private async computePipelineHealth() {
		...
		const lastExtractionTime = lastExtraction ? new Date(lastExtraction) : new Date(0);
	...
	private calculateGrowthRate(currentTotal: number, historicalData: any): number {
	...
	private async getHistoricalOverview() {
		try {
			...
			const object = await this.env.GENOMICS_DATA.get(`analytics/history/${dateKey}.json`);
			if (object) {
